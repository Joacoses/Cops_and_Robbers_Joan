...
public class Controller : MonoBehaviour
{
    ...
    public void InitAdjacencyLists()
    {
        ...
        for (int x = 0; x < Constants.NumTiles; x++)
        {
            ...
            {
                //casillas a las que se puede desplazar
                ...
                //si sobresalen por la izquierda, que no se muestre por la derecha
                int x2 = x + 1;
                ...
                //si sobresalen por la derecha, que no se muestre por la izquierda
                int x3 = x - 1;
                if (x % 8 == 0 && x != 0)
                {
    ...
    public void RobberTurn()
    {
        ...
        */
        ...
        //Obtenemos el numero aleatorio donde se va a mover
    ...
    public void FindSelectableTiles(bool cop)
    {
        ...
        while (nodes.Count != 0)
        {
            ...
            foreach (int adyacente in tiles[antes2].adjacency)
            {
                ...
                {
                    //la convertimos en true
                    tiles[adyacente].visited = true;
                    //+1 porque sino nos muestra todas las casillas como seleccionables
                    tiles[adyacente].distance = tiles[antes2].distance + 1;
                    //para que en diagonal haga dos movimientos, y no vaya directo
                    tiles[adyacente].parent = tiles[antes2];
                    //casillas adyacentes, sino solo se puede mover de 1 en 1 
                    nodes.Enqueue(tiles[adyacente]);

                    //si la distancia es <= 2
                    if (tiles[adyacente].distance <= 2)
                    {
                        //para el movimiento del caco, añadimos la casilla al array
                        if (cop == false)
                        {
